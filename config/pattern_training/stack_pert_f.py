...
attributes_info = {
    'age': {
        'short_name': 'ff',
        # 'model_path': '/tmp2/pfe/mfd/ff_l_3d0/checkpoint_epoch_0014.pth',
        'model_path': '/tmp2/pfe/lbc/ff_eta_0000d01_e04/checkpoint_epoch_0003.pth',
    ...
    for attribute, info in attributes_info.items():
        ...
        for attack_method, method_abbr in attack_method_abbr.items():
            for gamma in gamma_values_stage_1:
                ...
                config['training']['save_path'] = f"/tmp2/pfe/pattern/pert/stack2/{method_abbr}/{short_name}_g_{formatted_gamma}"
                ...
                directory = f"perturbation/stack2/{method_abbr}"
    ...
    for attribute, info in attributes_info.items():
        ...
        for attack_method, method_abbr in attack_method_abbr.items():
            ...
            for accuracy_goal in accuracy_goals_stage_2:
                ...
                config = base_config.copy()
                config['training']['save_path'] = f"/tmp2/pfe/pattern/pert/stack2/{method_abbr}/{short_name}_at_{formatted_accuracy_goal}"
                config['training']['gpu_setting'] = gpu_setting
                config['model']['model_path'] = model_path
                config['attack']['method'] = attack_method
    ...
    for attribute, info in attributes_info.items():
        ...
        for attack_method, method_abbr in attack_method_abbr.items():
            ...
            for gamma_adjust_factor in gamma_adjust_factors_stage_3:
                ...
                config = base_config.copy()
                config['training']['save_path'] = f"/tmp2/pfe/pattern/pert/stack2/{method_abbr}/{short_name}_af_{formatted_gamma_adjust_factor}"
                config['training']['gpu_setting'] = gpu_setting
                config['model']['model_path'] = model_path
                config['attack']['method'] = attack_method
                config['attack']['gamma_adjustment'] = 'dynamic'
