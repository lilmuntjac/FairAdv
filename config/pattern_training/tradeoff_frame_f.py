...
# }
thickness_value = [0.175, 0.2, 0.225]
gamma_values_stage_1 = [0.001]
# thickness_value = [0.05, 0.075, 0.1, 0.125, 0.15, 0.175, 0.2, 0.225]
...
def format_value(value, total_length=7):
    ...
    formatted_value = f"{value:.4f}".replace('.', 'd')
...
def generate_configs_stage_1():
    ...
    for attribute, info in attributes_info.items():
        ...
        gamma_max_length = max(len(f"{value:.2f}".replace('.', 'd')) for value in gamma_values_stage_1)
        thickness_max_length = max(len(f"{value:.3f}".replace('.', 'd')) for value in thickness_value)

        for attack_method, method_abbr in attack_method_abbr.items():
            for thickness in thickness_value:
                for gamma in gamma_values_stage_1:
                    formatted_gamma = format_value(gamma, gamma_max_length)
                    formatted_thickness = format_value(thickness, thickness_max_length)
                    config = base_config.copy()
                    config['training']['save_path'] = f"/tmp2/pfe/pattern/frame/tradeoff/{formatted_thickness}/{method_abbr}/{short_name}_g_{formatted_gamma}"
                    ...
                    config['attack']['frame_thickness'] = thickness
                    ...
                    directory = f"frame/tradeoff/{formatted_thickness}/{method_abbr}"
