...
accuracy_results_stage_2 = {
    ('Attractive', 'fairness constraint'): 0.82,
    ('Attractive', 'perturbed fairness constraint'): 0.82,
    ('Attractive', 'EquiMask fairness constraint'): 0.82,
    ('Attractive', 'EquiMask perturbed fairness constraint'): 0.82,
    ...
    for attribute, info in attributes_info.items():
        ...
        for attack_method, method_abbr in attack_method_abbr.items():
            ...
            for gamma_adjust_factor in gamma_adjust_factors_stage_3:
                ...
                config = base_config.copy()
                config['training']['save_path'] = f"/tmp2/pfe/pattern/frame/test/{method_abbr}/{short_name}_af_{formatted_gamma_adjust_factor}"
                config['training']['gpu_setting'] = gpu_setting
                config['model']['model_path'] = model_path
                config['attack']['method'] = attack_method
                config['attack']['gamma_adjustment'] = 'dynamic'
                ...
                directory = f"frame/test/{method_abbr}"
