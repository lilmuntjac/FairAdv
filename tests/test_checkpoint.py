...
class TestCheckpoint(unittest.TestCase):
    """
    A unittest class for verifying the accuracy of a trained model using saved checkpoints and statistics.
    This test ensures that the model's predictions and computed statistics when re-evaluated match
    previously computed results stored in checkpoint files.
    """

    @classmethod
    def setUpClass(cls):
        setup_start = time.perf_counter()
        config_path = './config/testing/cb_at_m_ckpt.yml'  # Specify the path to your configuration file
        ...
        if cls.epoch < 0: # no epoch is detected, get the epoch by the checkpoints name
            cls.epoch = cls.extract_epoch_number(cls.config['unit_test']['load_path'])
        model = cls.setup_model_class(cls.config, cls.device)
        cls.model, cls.loaded_stats = cls.load_checkpoint_and_stats(cls.config, model, cls.device)
    ...
    def test_checkpoint_accuracy(self):
        ...
        loaded_stats = self.loaded_stats['val'][self.epoch - 1]
        ...
        train_stats = self.loaded_stats['train'][self.epoch - 1]
        val_stats   = self.loaded_stats['val'][self.epoch - 1]
